...
class _Periodic:
    ...
    def arm_next(self, now: float) -> None:
        # Skip missed periods so a stall does not schedule a burst of catch-up work.
        self.next_t += self.period_s
        if self.next_t <= now:
            self.next_t = now + self.period_s


async def _opc_worker(
    *,
    opc: OpcUaBridge,
    m: Machine,
    timing_cfg,
    hmi_bridge: LocalBridge | None,
    enc_thread: ThreadedEncoder | None,
    enc_emu: SimEncoder | None,
    speed_outlier: SpeedOutlierReject,
    stop_event: asyncio.Event | None,
) -> None:
    """OPC poll/publish off the control path so awaits do not stretch the PID period."""
    p_opc_poll = _Periodic(timing_cfg.opcua_poll_period_s)
    p_opc_pub = _Periodic(timing_cfg.opcua_publish_period_s)
    min_sleep = min(timing_cfg.opcua_poll_period_s, timing_cfg.opcua_publish_period_s)
    ...
    while True:
        ...
        next_due = min(p_opc_poll.next_t, p_opc_pub.next_t)
        delay = max(0.001, min(min_sleep, next_due - monotonic()))
    ...
    try:
        ...
        try:
            while True:
                ...
                now = monotonic()
                loop_deadline = now + timing_cfg.task_period_s

                inp_gpio = gpio.read()
                if hmi_bridge is not None:
                    levels = gpio.read_levels()
                    if levels != last_gpio_levels:
                        last_gpio_levels = levels
                        hmi_bridge.publish_gpio_levels(levels)
                ...
                if p_task.due(now):
                    ...
                    m.telem.tension_n = m.calc_tension_n(
                        last_brake_pct, diameter_m=unwind_diameter_m
                    )
