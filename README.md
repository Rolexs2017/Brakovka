...
## Modbus RTU (карта регистров, которая сейчас используется)

Используется клиент `pymodbus`.

Частотник: пользовательская карта регистров.

- `0x2000` — регистр задания частоты, запись: `Hz * 100`
- `0x2001` — слово управления:
  - `0x0001` пуск вперёд
  - `0x0002` пуск назад
  - `0x0005` останов с замедлением (fallback по текущей логике)
- `0x2002` — слово состояния (чтение)
- `0x2003` — код неисправности (чтение)
