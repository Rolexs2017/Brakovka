...
### Фильтр команды на частотник

Чтобы снизить дрожание/дергание команды `VfdFreqCmd_Hz` (и, соответственно, запись в Modbus),
можно включить сглаживание:

- `machine.vfd_cmd_filter_tau_s` — постоянная времени low‑pass (сек). `0` — выключено.
- Телеметрия и команда в Modbus будут с фильтром.

Типичные значения для старта: `0.1 ... 0.3`.
...
Команды: `1` вперёд, `2` назад, `0` стоп.
