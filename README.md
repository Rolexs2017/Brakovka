...
Скорость считается из приращения метража за период задачи контроллера (`timing.task_period_s`), затем:

1. **Отсечение выбросов** (`SpeedOutlierReject` в `encoder.py`) — отбрасываются скачки вверх выше `max_ramp_speed_mpm × 1.5` и резкие скачки относительно последнего принятого значения; при выбросе держится предыдущая скорость.
2. **Скользящее среднее по 10 последним значениям** (`PID_REGULATOR_SPEED_AVG_N`).

Итоговое сглаженное значение идёт в телеметрию/HMI и в регулятор (PID / автонастройка).
