...
Скорость считается из приращения метража за **скользящее окно ~100 мс** (`SPEED_LENGTH_WINDOW_S` в `encoder.py`), а не за один период задачи — так меньше шум квантования AS5600. Далее:

1. **Отсечение выбросов** (`SpeedOutlierReject`) — скачки вверх выше `max_ramp_speed_mpm × 1.5` и резкие скачки относительно последнего принятого значения; при выбросе держится предыдущая скорость.
2. **Медиана по 11 последним значениям** (`PID_REGULATOR_SPEED_MEDIAN_N`) — это значение идёт в **PID / автонастройку**.
3. **Отдельный low-pass для экрана** (`SpeedDisplayFilter`, `SPEED_DISPLAY_TAU_S` ≈ 0.5 с) — сильнее сглаживает `Telemetry/Speed_mpm` и HMI, не влияет на регулятор.
