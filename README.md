...
Мгновенная скорость с энкодера (`Telemetry/Speed_mpm`) идёт на HMI без сглаживания.

Перед регулятором скорости (PID / автонастройка) применяется **скользящее среднее арифметическое по 10 последним значениям** (~100 мс при опросе 10 мс). Константа: `PID_REGULATOR_SPEED_AVG_N` в `encoder.py`.
