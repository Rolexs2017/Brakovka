...
# Hardware UART (serial0) + software GPIO DE.
set -euo pipefail
...
from brakovka_pi.config import RS485_DE_GPIO, load_runtime_config
...
print(
    ...
    f"unit_id={serial_cfg.unit_id} DE=GPIO{serial_cfg.rs485_de} software "
...
async def main() -> int:
    ...
    if not vfd._connected:
        ...
        return 3
    print("OK: port open")
    print(f"DE gpio ready: {getattr(vfd, 'de_patched', False)}")
